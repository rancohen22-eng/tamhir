...
- **קבצים:** אזור המציג `booking_files` (הורדה/תצוגה), עם **לוגו טלטוס** בראש כרטיס ההזמנה כשמשויך סוכן (סעיף 8).
...
- **לוגו טלטוס (co-branding):**
  - העלו `assets/teltos-logo.jpeg` ל-`agents.logo_blob` של הסוכן טלטוס (דרך Grid הניהול או File Browse).
...
2. התחבר כ-`teltos` → פתח את הבקשה → הגש הצעת מחיר + העלה צילום. ודא `QUOTE_RECEIVED` והתראה (פעמון) ל-`initiator_acmi`.
3. התחבר כ-`approver_acmi` → אשר. ודא `APPROVED` והתראה לסוכן.
4. חזור כ-`teltos` → כרטס: PNR + מזהה חד-ערכי + מסמך. ודא `TICKETED` והתראות למזמין+מאשר.
5. בפירוט ההזמנה — ודא **ציר זמן** מלא (מי/מתי) והקבצים המצורפים.
6. **ביטול:** כ-`initiator_acmi` בקש ביטול → כ-`teltos` אשר ביטול → ודא `CANCELLED` + התראות.
