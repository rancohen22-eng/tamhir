...
BEGIN
  ...
  -- ניסיונות התחברות (מוצלח/כושל) — לנעילת חשבון מפני brute-force וניטור
  try('CREATE TABLE login_attempts (
  ...
  -- לוג מיילים שנשלחו + מעקב קריאה (פיקסל 1x1). opened/opened_at מתעדכנים כשהמייל נטען.
  try('CREATE TABLE email_log (
        log_id     NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY PRIMARY KEY,
        token      VARCHAR2(70) UNIQUE,
        booking_id NUMBER,
...
CREATE OR REPLACE PACKAGE api_pkg AS
  ...
  FUNCTION  admin_email_log (p_token VARCHAR2) RETURN CLOB;
  FUNCTION  admin_login_attempts (p_token VARCHAR2) RETURN CLOB;
  ...
  -- פיקסל מעקב קריאת מייל — מסמן נקרא ומחזיר GIF 1x1
  FUNCTION  track_open    (p_t VARCHAR2) RETURN BLOB;
  -- מאשר לכל מחלקה
...
CREATE OR REPLACE PACKAGE BODY api_pkg AS
  ...
  -- IP של הלקוח (best-effort; ייתכן שיראה את ה-gateway מאחורי פרוקסי).
  -- קריאה דינמית ל-OWA_UTIL כדי שלא תיווצר תלות קומפילציה (login חייב להישאר תקין תמיד).
  FUNCTION client_ip RETURN VARCHAR2 IS
    l VARCHAR2(200);
  BEGIN
    BEGIN EXECUTE IMMEDIATE 'begin :x := owa_util.get_cgi_env(''X-Forwarded-For''); end;' USING OUT l;
    EXCEPTION WHEN OTHERS THEN l := NULL; END;
    IF l IS NULL THEN
      BEGIN EXECUTE IMMEDIATE 'begin :x := owa_util.get_cgi_env(''REMOTE_ADDR''); end;' USING OUT l;
      EXCEPTION WHEN OTHERS THEN l := NULL; END;
    END IF;
    RETURN SUBSTR(l, 1, 60);
  EXCEPTION WHEN OTHERS THEN RETURN NULL;
  END client_ip;

  -- התראת אבטחה לכל מנהלי המערכת (נעילת חשבון עקב ריבוי כשלונות)
  PROCEDURE sec_alert_admins (p_username VARCHAR2, p_ip VARCHAR2) IS
  BEGIN
    FOR a IN (SELECT DISTINCT u.email FROM app_users u JOIN user_roles r ON r.user_id = u.user_id
               WHERE r.role_code = 'ADMIN' AND u.email IS NOT NULL AND u.is_active = 'Y') LOOP
      BEGIN
        ...
        APEX_MAIL.SEND(p_to => a.email, p_from => booking_pkg.g_mail_from,
          p_subj => 'ארקיע — התראת אבטחה: חשבון ננעל עקב ריבוי כשלונות',
          p_body => TO_CLOB('זוהו 5 ניסיונות התחברות כושלים לחשבון "'||p_username||'" והוא ננעל ל-15 דקות.'||CHR(10)||
                    'IP: '||NVL(p_ip,'לא ידוע')||CHR(10)||'זמן: '||TO_CHAR(SYSTIMESTAMP,'YYYY-MM-DD HH24:MI:SS')));
      EXCEPTION WHEN OTHERS THEN NULL; END;
    END LOOP;
    BEGIN APEX_MAIL.PUSH_QUEUE; EXCEPTION WHEN OTHERS THEN NULL; END;
  END sec_alert_admins;
  ...
  FUNCTION login (p_username VARCHAR2, p_password VARCHAR2) RETURN CLOB IS
    ...
    l_notify VARCHAR2(1); l_ip VARCHAR2(60); l_fails PLS_INTEGER;
  BEGIN
    l_ip := client_ip;
    -- נעילה: 5+ כשלונות ל-15 הדקות האחרונות → חסום (מונע brute-force)
    SELECT COUNT(*) INTO l_fails FROM login_attempts
      WHERE LOWER(username) = LOWER(p_username) AND success = 'N'
        AND attempted_at > SYSTIMESTAMP - INTERVAL '15' MINUTE;
    IF l_fails >= 5 THEN
      INSERT INTO login_attempts (username, ip, success) VALUES (LOWER(p_username), l_ip, 'N'); COMMIT;
      RETURN err('locked');
    END IF;
    IF NOT booking_pkg.authenticate(p_username, p_password) THEN
      INSERT INTO login_attempts (username, ip, success) VALUES (LOWER(p_username), l_ip, 'N');
      COMMIT;
      -- אם הכשלון הזה השלים ל-5 — נעילה + התראת אדמין (פעם אחת)
      IF l_fails + 1 = 5 THEN sec_alert_admins(LOWER(p_username), l_ip); END IF;
      RETURN err('bad_credentials');
    END IF;
    INSERT INTO login_attempts (username, ip, success) VALUES (LOWER(p_username), l_ip, 'Y');
        ...
        BEGIN
          ...
          APEX_MAIL.SEND(p_to => a.email, p_from => booking_pkg.g_mail_from,
            ...
            p_body_html => booking_pkg.booking_email_html(p_id, a.pref_lang)
                           || booking_pkg.track_pixel(p_id, a.email, a.user_id, 'AGENT_REQUEST', 'בקשה חדשה להצעת מחיר #'||p_id));
        EXCEPTION WHEN OTHERS THEN NULL; END;
      END IF;
    END LOOP;
    BEGIN APEX_MAIL.PUSH_QUEUE; EXCEPTION WHEN OTHERS THEN NULL; END;
        ...
        BEGIN
          ...
          l_mail_id := APEX_MAIL.SEND(p_to => a.email, p_from => booking_pkg.g_mail_from,
            ...
            p_body_html => booking_pkg.booking_email_html(p_id, a.pref_lang, l_url)
                           || booking_pkg.track_pixel(p_id, a.email, a.user_id, 'APPROVER_QUOTE', 'נדרש אישור, הזמנה #'||p_id));
  ...
  -- פיקסל מעקב: מסמן את המייל כ"נקרא" ומחזיר GIF שקוף 1x1. autonomous — לא תלוי ב-caller.
  FUNCTION track_open (p_t VARCHAR2) RETURN BLOB IS
    PRAGMA AUTONOMOUS_TRANSACTION;
    l_gif BLOB := TO_BLOB(HEXTORAW('47494638396101000100800000000000FFFFFF21F90401000000002C00000000010001000002024C01003B'));
  BEGIN
    UPDATE email_log
       SET opened = 'Y', opened_at = NVL(opened_at, SYSTIMESTAMP), open_count = NVL(open_count,0) + 1
     WHERE token = p_t;
    COMMIT;
    RETURN l_gif;
  EXCEPTION WHEN OTHERS THEN ROLLBACK; RETURN l_gif;
  END track_open;
  ...
  BEGIN
    IF p_password IS NOT NULL AND LENGTH(p_password) < 8 THEN RETURN err('weak_password'); END IF;
    ...
    IF p_email IS NOT NULL THEN
      ...
      BEGIN
        ...
        APEX_MAIL.SEND(p_to => p_email, p_from => booking_pkg.g_mail_from,
          ...
          p_body_html => l_html || booking_pkg.track_pixel(NULL, p_email, l_new, 'WELCOME', 'ברוכים הבאים — פרטי כניסה'));
        BEGIN APEX_MAIL.PUSH_QUEUE; EXCEPTION WHEN OTHERS THEN NULL; END;
        l_emailed := 'true';
  ...
  FUNCTION admin_setpw (p_token VARCHAR2, p_username VARCHAR2, p_password VARCHAR2) RETURN CLOB IS
    l_uid NUMBER := admin_guard(p_token);
  BEGIN
    IF p_password IS NULL OR LENGTH(p_password) < 8 THEN RETURN err('weak_password'); END IF;
  ...
  BEGIN
    ...
    IF l_email IS NOT NULL THEN
      ...
      BEGIN
        BEGIN APEX_UTIL.SET_SECURITY_GROUP_ID(APEX_UTIL.FIND_SECURITY_GROUP_ID('ARKIA')); EXCEPTION WHEN OTHERS THEN NULL; END;
        APEX_MAIL.SEND(p_to => l_email, p_from => booking_pkg.g_mail_from, p_subj => l_subj,
          p_body      => TO_CLOB(l_body),
          p_body_html => TO_CLOB('<div dir="rtl" style="font-family:Heebo,Arial,sans-serif">'||REPLACE(l_body,CHR(10),'<br>')||'</div>')
                         || booking_pkg.track_pixel(NULL, l_email, p_user_id, 'RESET', l_subj));
        BEGIN APEX_MAIL.PUSH_QUEUE; EXCEPTION WHEN OTHERS THEN NULL; END;
  ...
  -- לוג מיילים שנשלחו + סטטוס קריאה (300 האחרונים)
  FUNCTION admin_email_log (p_token VARCHAR2) RETURN CLOB IS
    l_uid NUMBER := admin_guard(p_token);
  BEGIN
    APEX_JSON.initialize_clob_output;
    APEX_JSON.open_object; APEX_JSON.write('ok', TRUE); APEX_JSON.open_array('emails');
    FOR r IN (SELECT e.recipient, e.kind, e.subject, e.booking_id,
                     TO_CHAR(e.sent_at,'YYYY-MM-DD HH24:MI') sent,
                     e.opened, TO_CHAR(e.opened_at,'YYYY-MM-DD HH24:MI') opened_at, NVL(e.open_count,0) oc
                FROM email_log e ORDER BY e.sent_at DESC FETCH FIRST 300 ROWS ONLY) LOOP
      APEX_JSON.open_object;
      APEX_JSON.write('to', r.recipient); APEX_JSON.write('kind', r.kind);
      APEX_JSON.write('subject', r.subject); APEX_JSON.write('booking_id', r.booking_id);
      APEX_JSON.write('sent', r.sent); APEX_JSON.write('opened', r.opened);
      APEX_JSON.write('opened_at', r.opened_at); APEX_JSON.write('open_count', r.oc);
      APEX_JSON.close_object;
    END LOOP;
    APEX_JSON.close_array; APEX_JSON.close_object;
    RETURN APEX_JSON.get_clob_output;
  ...
  -- ניסיונות התחברות אחרונים (מוצלח/כושל) + IP — לניטור אבטחה
  FUNCTION admin_login_attempts (p_token VARCHAR2) RETURN CLOB IS
    l_uid NUMBER := admin_guard(p_token);
  BEGIN
    APEX_JSON.initialize_clob_output;
    APEX_JSON.open_object; APEX_JSON.write('ok', TRUE); APEX_JSON.open_array('attempts');
    FOR r IN (SELECT username, ip, success, TO_CHAR(attempted_at,'YYYY-MM-DD HH24:MI:SS') atx
                FROM login_attempts ORDER BY attempted_at DESC FETCH FIRST 200 ROWS ONLY) LOOP
      APEX_JSON.open_object; APEX_JSON.write('username', r.username); APEX_JSON.write('ip', r.ip);
      APEX_JSON.write('success', r.success); APEX_JSON.write('at', r.atx); APEX_JSON.close_object;
    END LOOP;
    APEX_JSON.close_array; APEX_JSON.close_object;
    RETURN APEX_JSON.get_clob_output;
  EXCEPTION WHEN OTHERS THEN APEX_JSON.free_output; RETURN err(SQLERRM);
  END admin_login_attempts;
...
BEGIN
  ...
  -- track (GET, media — פיקסל מעקב קריאת מייל; מסמן נקרא ומחזיר GIF שקוף 1x1)
  ORDS.DEFINE_TEMPLATE(p_module_name => 'arkia.api', p_pattern => 'track');
  ORDS.DEFINE_HANDLER(p_module_name=>'arkia.api', p_pattern=>'track', p_method=>'GET',
    p_source_type=>ORDS.source_type_media,
    p_source=>q'[SELECT 'image/gif' AS content_type, api_pkg.track_open(:t) AS content FROM dual]');
  ...
  -- admin/attempts (POST = ניסיונות התחברות מוצלח/כושל + IP)
  ORDS.DEFINE_TEMPLATE(p_module_name => 'arkia.api', p_pattern => 'admin/attempts');
  ORDS.DEFINE_HANDLER(p_module_name=>'arkia.api', p_pattern=>'admin/attempts', p_method=>'POST',
    p_source_type=>ORDS.source_type_plsql,
    p_source=>q'[BEGIN api_pkg.emit(api_pkg.admin_login_attempts(:token)); EXCEPTION WHEN OTHERS THEN api_pkg.emit(api_pkg.err(SQLERRM)); END;]');

  -- admin/emaillog (POST = לוג מיילים שנשלחו + סטטוס קריאה)
  ORDS.DEFINE_TEMPLATE(p_module_name => 'arkia.api', p_pattern => 'admin/emaillog');
  ORDS.DEFINE_HANDLER(p_module_name=>'arkia.api', p_pattern=>'admin/emaillog', p_method=>'POST',
    p_source_type=>ORDS.source_type_plsql,
    p_source=>q'[BEGIN api_pkg.emit(api_pkg.admin_email_log(:token)); EXCEPTION WHEN OTHERS THEN api_pkg.emit(api_pkg.err(SQLERRM)); END;]');
